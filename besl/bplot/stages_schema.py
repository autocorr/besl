...
def plot_schema(x=6, x_off=-1, y_off=-0.7, outname='stages_schema'):
    """
    Plot a schematic diagram of the BGPS evolutionary stages flag groups.
    ...
    pgm.add_node(daft.Node('bolocat_t', '', x / 3, bo_y + box_dy + 0.1,
        scale=0))
    ...
    wide = 1.5
    f_x, f_y = 0.16 * x, 2.4
    pgm.add_node(daft.Node('hg70', 'Hi-GAL \n 70 ${\\rm \\mu m}$', 0 * f_x,
        f_y, scale=wide, offset=[0, -2]))
    pgm.add_node(daft.Node('ir', 'IR / \n YSO', 1 * f_x, f_y, scale=wide))
    pgm.add_node(daft.Node('h2o', 'H$_2$O', 2 * f_x, f_y, scale=wide,
        offset=[0, -2]))
    pgm.add_node(daft.Node('ch3oh', 'CH$_3$OH', 3 * f_x, f_y, scale=wide,
        offset=[0, -2]))
    pgm.add_node(daft.Node('uchii', 'UCHII', 4 * f_x, f_y, scale=wide))
    ...
    pgm.add_node(daft.Node('akari', r'AKARI', 4 * f_x, f_y - 1.05 * f_x,
        scale=wide, observed=True))
    pgm.add_node(daft.Node('mipsgal', r'MIPS', 4 * f_x, f_y - 2.1 * f_x,
        scale=wide, observed=True))
    pgm.add_edge('uchii', 'akari', directed=False,
        plot_params={'linestyle': 'dotted'})
    ...
    # Save Figure
    pgm.render()
    pgm.figure.savefig(outname + '.pdf')
    pgm.figure.savefig(outname + '.png', dpi=150)
    return pgm
